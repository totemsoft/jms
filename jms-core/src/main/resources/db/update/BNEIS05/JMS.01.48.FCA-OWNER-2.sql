...
--	4				Property Tenant
--	5				Alternate
IF EXISTS (SELECT name  FROM sysobjects WHERE  name = N'V_FCA_OWNER_2' AND type = 'V')
    DROP VIEW V_FCA_OWNER_2
GO
CREATE VIEW V_FCA_OWNER_2
AS
SELECT fca AS FCA_NO,
      ...
      email AS EMAIL,
      fax AS FAX
FROM [BB1911\SAMC].[AIMS].[dbo].[tbl_bc]
WHERE [FCA] < '90000-01'
AND LEN([FCA]) = 8
AND [abn] IS NOT NULL
AND [companyname] IS NOT NULL
AND [bccompanyname] IS NOT NULL
AND [ctsnumber] IS NOT NULL
...
GO
-- SELECT * FROM V_FCA_OWNER_2 ORDER BY FCA_NO

IF EXISTS (SELECT name  FROM sysobjects WHERE  name = N'sps_fca_owner_2' AND type = 'P')
    DROP PROCEDURE sps_fca_owner_2
GO
CREATE PROCEDURE sps_fca_owner_2
AS
BEGIN TRANSACTION;
BEGIN TRY
