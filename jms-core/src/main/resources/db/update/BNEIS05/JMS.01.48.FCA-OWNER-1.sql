...
--	4				Property Tenant
--	5				Alternate
IF EXISTS (SELECT name  FROM sysobjects WHERE  name = N'V_FCA_OWNER_1' AND type = 'V')
    DROP VIEW V_FCA_OWNER_1
GO
CREATE VIEW V_FCA_OWNER_1
AS
SELECT fca AS FCA_NO,
      ...
      email AS EMAIL,
      fax AS FAX
FROM [BB1911\SAMC].[AIMS].[dbo].[tbl_oi] 
WHERE fca < '90000-01' AND LEN(fca) = 8
GO
-- SELECT * FROM V_FCA_OWNER_1 ORDER BY FCA_NO

IF EXISTS (SELECT name  FROM sysobjects WHERE  name = N'sps_fca_owner_1' AND type = 'P')
    DROP PROCEDURE sps_fca_owner_1
GO
CREATE PROCEDURE sps_fca_owner_1
AS
BEGIN TRANSACTION;
BEGIN TRY
