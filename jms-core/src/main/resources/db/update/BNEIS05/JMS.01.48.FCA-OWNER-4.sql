...
--	4				Property Tenant
--	5				Alternate
IF EXISTS (SELECT name  FROM sysobjects WHERE  name = N'V_FCA_OWNER_4' AND type = 'V')
    DROP VIEW V_FCA_OWNER_4
GO
CREATE VIEW V_FCA_OWNER_4
AS
SELECT fca AS FCA_NO,
      ...
      email AS EMAIL,
      fax AS FAX
FROM [BB1911\SAMC].[AIMS].[dbo].[tbl_pt]
WHERE [FCA] < '90000-01'
AND LEN([FCA]) = 8
...
GO
-- SELECT * FROM V_FCA_OWNER_4 ORDER BY FCA_NO

IF EXISTS (SELECT name  FROM sysobjects WHERE  name = N'sps_fca_owner_4' AND type = 'P')
    DROP PROCEDURE sps_fca_owner_4
GO
CREATE PROCEDURE sps_fca_owner_4
AS
BEGIN TRANSACTION;
BEGIN TRY
