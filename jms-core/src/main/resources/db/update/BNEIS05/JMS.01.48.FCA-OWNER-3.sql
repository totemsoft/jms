...
--	4				Property Tenant
--	5				Alternate
IF EXISTS (SELECT name  FROM sysobjects WHERE  name = N'V_FCA_OWNER_3' AND type = 'V')
    DROP VIEW V_FCA_OWNER_3
GO
CREATE VIEW V_FCA_OWNER_3
AS
SELECT fca AS FCA_NO,
      ...
      email AS EMAIL,
      fax AS FAX
FROM [BB1911\SAMC].[AIMS].[dbo].[tbl_pm]
WHERE [FCA] < '90000-01'
AND LEN([FCA]) = 8
...
GO
-- SELECT * FROM V_FCA_OWNER_3 ORDER BY FCA_NO

IF EXISTS (SELECT name  FROM sysobjects WHERE  name = N'sps_fca_owner_3' AND type = 'P')
    DROP PROCEDURE sps_fca_owner_3
GO
CREATE PROCEDURE sps_fca_owner_3
AS
BEGIN TRANSACTION;
BEGIN TRY
